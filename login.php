<?php
 ...
 if(array_key_exists('username',$_POST) OR array_key_exists('password', $_
POST))
 {
 if ($_POST['username']=='')
   {
 ...
 $query3 ="SELECT `EmailId` FROM `signup` WHERE username='".mysqli_real_escape_string($link, $_POST['username'])."' AND password= '".$_POST['password']."' limit 1 ";
 ...
 $query6 ="SELECT `RoomNo` FROM `signup` WHERE username='".mysqli_real_escape_string($link, $_POST['username'])."' AND password= '".$_POST['password']."' limit 1 ";
 ...
 echo "INVALID CREDENTIALS, PLEASE GIVE VALID USERNAME AND PASSWORD TO
LOG IN";
 }
...
background: linear-gradient(-45deg, #efa752, #ef304e, #13b6e5, #29e4fb);
...
@keyframes gradient {
 ...
 <a href="webpageusingcss.php"><img id="logoimg" src="logo.png"></a>

 <div id="login" >
 <img id="logo2" src="logo2.png">
 <form id="form" method="POST" action="#">
 <p><input type="text" placeholder="username" name="username" require
d></input></p>
 <p><input type="password" placeholder="password" name="password" req
uired></p>
